...
(define outer-layer
  (lambda (statement-list environment throw)
    (cond
      ...
      ((eq? 'var (statement-type (car statement-list))) (outer-layer (cdr statement-list) (interpret-declare (car statement-list) environment throw) throw))
      ((eq? 'function (statement-type (car statement-list))) (outer-layer (cdr statement-list) (insert-function (car statement-list) environment) throw))
...
(define execute-main
  (lambda (environment return break continue throw)
    (interpret-statement-list (func-body 'main environment throw) (push-frame environment) return break continue throw)))
...
(define generate-func-environment
  (lambda (name closure call environment throw)
    (cons (bind-params name environment call throw) (get-scope name environment))))
...
(define eval-function-helper
  (lambda (expr environment return break continue throw)
    (interpret-statement-list (func-body (get-func-name expr) environment throw)
                              (get-scope (get-func-name expr) (func-closure (get-func-name expr) environment) throw)
                              return break continue throw)))
